...
class MarkerInfo extends Component {
  ...
  render() {
    ...
    return (
      <Marker
        icon={
          showInfoId === placeId && action === 'open'
            ? { url: IconSel }
            : { url: IconDef }
        }
        key={placeId}
      ...
      >
        {showInfoId === placeId &&
          ...
          action === 'open' && (
            ...
            >
              {error ? (
                <Error message="There was an error while fetching this place's data. Please try again later." />
              ) : (
                <div className="details-place" tabIndex="0" key={placeId}>
                  ...
                  <p className="details-address">
                    {placeDetails.location.address || 'Address not found'}
                  </p>
                  <div
                  ...
                  <div
                    ...
                    title={`The price is ${placeDetails.price.message}`}
                  >
                    <span aria-hidden="true">
                      {placeDetails.attributes.groups['0'].summary}
                    </span>
                  </div>
                  ...
                  <div className="details-img">
                    <img
                      src={`${placeDetails.bestPhoto.prefix}width150${
                        placeDetails.bestPhoto.suffix
                      }`}
                      alt={`Best of ${placeDetails.name}`}
                    />
                  </div>
                  <a
                    className="details-more"
                    href={placeDetails.canonicalUrl}
